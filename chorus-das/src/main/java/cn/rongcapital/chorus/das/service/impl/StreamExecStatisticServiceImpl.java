...
import cn.rongcapital.chorus.das.dao.StreamExecStatisticMapper;
 ...
 */
@Service(value = "StreamExecStatisticService")
@Transactional
public class StreamExecStatisticServiceImpl implements StreamExecStatisticService {
    private static Logger logger = LoggerFactory.getLogger(StreamExecStatisticServiceImpl.class);
    private static final DateFormat df = new SimpleDateFormat("yyyy-MM-dd");

    @Autowired(required = false)
    private StreamExecStatisticMapper streamExecStatisticMapper;
    
    @Autowired(required = false)
    private ProjectInfoMapper projectInfoMapper;

    @Override
    public List<Map<String, Object>> getStreamExecDist(long projectId, int recentDayNum) {
        Calendar cal = Calendar.getInstance();
        cal.add(Calendar.DATE, -recentDayNum);
        List<Map<String, Object>> list = streamExecStatisticMapper.getStreamExecDist(projectId, df.format(cal.getTime()));
    ...
    @Override
    public boolean addStreamJobExecStatistic(StreamExecStatistic streamExecStatistic) {
        return streamExecStatisticMapper.add(streamExecStatistic) > 0;
    }
